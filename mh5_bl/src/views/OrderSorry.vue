  ...
  <div class="order">
    <div class="order_margin">
      ...
      <van-grid>
        ...
        >
          <div slot="footer">
            <p class="shu">×{{orderSorry_cart}}</p>
            <p class="mian">免邮</p>
            <hr class="hr" />
            <p>
              合计:
              <span class="qian">￥{{item.price*orderSorry_cart}}</span>
            </p>
          </div>
          <div slot="tags">
...
export default {
  ...
  methods: {
    ...
    order_shop() {
      // 转换成数组形式
      let goods_id = localStorage.getItem("orderSorry_id");
      // 定义一个变量接受goods_id
      let str = [];
      // 序号每一个goods_id
    ...
    onSubmit() {
      ...
      this.goodsData.forEach((element, i) => {
        carts.push({
          goods_id: element.id,
          buy_count: this.orderSorry_cart
        });
      });
      // console.log(carts);
      this.$http
  ...
  computed: {
    ...
    totalPrices() {
      ...
      this.goodsData.forEach(item => {
        // console.log(item);
        // 总价钱
        sum += item.price * this.orderSorry_cart;
      });
      return sum * 100;
    }
  },
